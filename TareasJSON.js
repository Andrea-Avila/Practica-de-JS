...
//Reviso el ID del producto a elimir en cada uno de los inidces de mi arregrlo si encuentoro un indice que dentro tenga un objeto que conincida con el id que uiero borrarm entonces lo elimino

let indiceProductoAEliminar =carritoDeCompras.findIndex(function
    (producto){
        return producto.id ===productoAEliminar; //ture o false
    });

//Verifica si el indice del elemnto que se encontro es valido. con el findIndex por a devolver el primer indice del elemento que cumpla con la condicion. Si no sse encuentra el elemnto que conicida con la condicion entonces vamsos a devolver un -1 esto para evitar borrar un elemento que no sea (pensando que tenemos un indice 0)

    if (indiceProductoAEliminar !== -1){  //0,1,2,3, dispara un true y se activa el if. Que el .1 dispara un false y no se activa el if
    carritoDeCompras .splice (indiceProductoAEliminar,1)
        
    }
console.log("Tienes: ", carritoDeCompras.length," productos en tu carrito");

/*El merodo forEach es un metodo de array de JS, que nos va a ayudar a poder ejecura ua funcion en cada elemento de nuestro array.
...
//reiso el total de mi carrito de compras
